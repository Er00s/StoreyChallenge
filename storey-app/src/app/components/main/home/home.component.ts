...
import { switchMap, startWith, debounceTime, takeUntil, tap } from 'rxjs/operators';
...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.stateService.selectedServiceType$.pipe(
  ...
  openAssociateModal(): void {
    ...
    this.mockDataService.getAssociableSupplies(currentSupplyIds).subscribe(supplies => {
  ...
  associateNewSupply(supply: Supply): void {
    ...
    if (currentData) {
      ...
      const updatedData: CardData = { ...currentData, supplies: updatedSupplies };
